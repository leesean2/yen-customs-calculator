 ...
 *          it 장바구니 — "가격:품목id[:hs부호:세율%]" 콤마 목록. 상품이 2개 이상이거나
 *             HS부호 정확 세율을 적용한 상품이 있을 때만 쓴다.
...
export function readShareParams(search = window.location.search) {
  ...
  // it: "가격:품목id[:hs부호:세율%],..." — 형식이 어긋난 항목은 버리고 유효 항목만 복원.
  // hs 세그먼트는 HS부호 정확 관세율을 적용한 상품의 스냅샷(받는 쪽 재조회 불필요)
  const it = (q.get("it") ?? "")
    .split(",")
    .slice(0, MAX_CART_ITEMS)
    .map((pair) => {
      const [p, c, hs, d] = pair.split(":");
      const v = parseFloat(p);
      if (!Number.isFinite(v) || v < 0 || !CATEGORIES.some((x) => x.id === c)) return null;
      const rate = parseFloat(d);
      const withHs = /^\d{10}$/.test(hs ?? "") && Number.isFinite(rate) && rate >= 0;
      return withHs ? { p: String(v), c, hs, d: rate } : { p: String(v), c };
    })
    .filter(Boolean);
  return {
    p: numStr("p"),
    ...
    it: it.length ? it : null, // 상품 1개여도 hs 세율이 붙어 있으면 it 경로를 쓴다
 ...
 *  items: [{ price, catId, hsRate? }] — 첫 상품은 p·c로, 2개 이상이거나 HS 세율
 *  적용 상품이 있으면 전체를 it로도 담는다
...
export function buildShareUrl({ items, localShip, intlShip, countryId, jr, ur, originRate }) {
  ...
  if (items.length > 1 || items.some((x) => x.hsRate)) {
    q.set("it", items.slice(0, MAX_CART_ITEMS)
      .map((x) => `${parseFloat(x.price) || 0}:${x.catId}${x.hsRate ? `:${x.hsRate.hs}:${x.hsRate.rate}` : ""}`)
      .join(","));
  }
