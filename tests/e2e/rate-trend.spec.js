...
test("R6. 조회 실패 후 '다시 시도'가 차트와 1년 백분위를 함께 살린다", async ({ page }) => {
  await blockExternal(page);
  let fail = true; // 첫 조회는 전부 실패시키고, 재시도부터 정상 응답
  await page.route(/api\.frankfurter\.dev\/v1\/\d{4}-\d{2}-\d{2}\.\./, (r) => {
  ...
  await page.goto("/");
  await page.getByRole("button", { name: /환율 추이 · 이상 감지 보기/ }).click();
  await expect(page.getByText("추이를 불러오지 못했습니다.")).toBeVisible();
  await expect(page.getByText(/최근 1년 중/)).toBeHidden();

  fail = false;
  await page.getByRole("button", { name: "다시 시도" }).click();
  await expect(page.getByText("최고 980원")).toBeVisible();
  // 백분위 배지도 같은 재시도로 함께 살아난다
  await expect(page.getByText(/최근 1년 중 하위 50% — 중간 수준입니다/)).toBeVisible();
});
...
test("R5. 어느 탭에서든 보이고, 열림 상태는 새로고침 후에도 유지된다", async ({ page }) => {
  ...
  await page.reload();
  await expect(page.getByText("최고 980원")).toBeVisible();
  await expect(page.getByRole("button", { name: /환율 추이 · 이상 감지 접기/ })).toBeVisible();

  // 접으면 차트가 사라진다
  await page.getByRole("button", { name: /환율 추이 · 이상 감지 접기/ }).click();
