...
/* ──────────────────────────────────────────────
   해외 배송 통관조회 E2E — /api/cargo-status를 모킹해 조회→단계 표시,
   어느 탭에서든 보이는지, 오류 처리까지 검증한다 (실 API 스키마는 배포 시 검증됨).
   ────────────────────────────────────────────── */

async function openPanel(page) {
  await openWithRates(page);
  await page.getByRole("button", { name: /해외 배송 통관조회 보기/ }).click();
}
...
test("어느 탭에서든 보이고, 열림 상태는 새로고침 후에도 유지된다", async ({ page }) => {
  await openPanel(page);
  await expect(page.getByLabel("화물관리번호")).toBeVisible();

  await page.getByRole("button", { name: "여행자" }).click();
  await expect(page.getByLabel("화물관리번호")).toBeVisible();

  await page.reload();
  await expect(page.getByRole("button", { name: /해외 배송 통관조회 접기/ })).toBeVisible();
  await expect(page.getByLabel("화물관리번호")).toBeVisible();
});
