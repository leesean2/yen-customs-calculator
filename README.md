...
`CargoStatus.jsx` — 계산 결과와 무관한 독립 조회라 다른 탭들과 나란히 여섯 번째 탭으로 둔다
(처음엔 환율 설정 아래 전역 접이식 패널이었으나, 작은 링크라 놓치기 쉬워 탭으로 승격했다).
특송업체·판매자가 알려주는 화물관리번호("통관번호")를 입력하면 `/api/cargo-status`
(UNI-PASS 화물통관진행정보, `UNIPASS_CARGO_API_KEY` 필요)가 지금 통관이 어느 단계인지
(적재화물목록제출→수입신고수리→반출 등) 순서대로 보여준다.
